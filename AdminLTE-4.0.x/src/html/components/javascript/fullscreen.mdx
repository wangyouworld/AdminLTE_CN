...
| `inFullScreen()` | `void` | 调用 `document.documentElement.requestFullscreen()`，切换最大化/最小化图标上的 `.d-none` 类。触发 `maximized.lte.fullscreen` 事件。 |
| `outFullscreen()` | `void` | 调用 `document.exitFullscreen()`，切换最大化/最小化图标上的 `.d-none` 类。触发 `minimized.lte.fullscreen` 事件。 |

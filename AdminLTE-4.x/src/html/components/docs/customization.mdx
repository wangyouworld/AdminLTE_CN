...
AdminLTE 修改了大约十个 Bootstrap 默认值，位于 [`src/scss/_bootstrap-overrides.scss`](https://github.com/ColorlibHQ/AdminLTE/blob/master/src/scss/_bootstrap-overrides.scss)；其余所有 Bootstrap 变量（按钮颜色、间距比例、响应式断点）均保持其原始默认值。所有值均使用 `!default`，因此请在导入前于自己的 SCSS 文件中进行覆盖：
...
> **注意：** 编译 Sass 源码需要先安装 `bootstrap`——
> 自 4.1 版起，它是 `admin-lte` 的对等依赖项
> （npm 会随之一同自动安装）。请确保
>  Sass 编译器能识别 `bootstrap/...` 这类导入路径，方法是将 `node_modules` 加入加载路径。
> 例如通过命令行参数 `sass --load-path=node_modules`，或在 Vite 中配置
